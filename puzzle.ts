...
async function loadNextPuzzle() {
  ...
  if (typeof nextItem === "string") {
    ...
    try {
      ...
      puzzleChess.undo(); // back up one move

      p = {
        id: rawPuzzle.puzzle.id,
        rating: rawPuzzle.puzzle.rating,
        fen: puzzleChess.fen(),
        lastMove: lastMove, // {from,to,promotion}
  ...
  if (isStreakPuzzle) {
    // Start puzzle with the PGN last move visually replayed
    startPuzzle(p.fen, p.lastMove);
  } else {
    // Start puzzle with first opponent move from solution line
...
function startPuzzle(initFen: string, oppMove: any) {
  ...
  setTimeout(() => {
    if (typeof oppMove === "string") {
      // storm/local mode: oppMove is UCI string
      makeMove(oppMove, false);
    } else {
      // streak mode: oppMove is {from,to,promotion}
      chess.move(oppMove);
